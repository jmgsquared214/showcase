...
package com.smartgwt.sample.showcase.client.dataintegration.java.scripting;
...
public class ScriptingUserSpecificDataSample extends ShowcasePanel {
    private static final String DESCRIPTION = "<p>This example shows the use of some simple user-written " +
            "server code in conjunction with Smart GWT databound dragging features, the Smart GWT SQL DataSource" +
            " and Server Scripting to implement a simple, but secure, shopping cart example.</p>" +
            "<p>Via Server-side Scripting, the cartItem DataSource declares that all DataSource operations " +
            "should go through a custom server script <code>DataSource.script</code> before proceeding to read or write to the" +
            " database.   <code>DataSource.script</code> adds the current sessionId to the DSRequest, so that the user can only" +
            " read and write to their own shopping cart.</p>" +
            "<p>Drag items from the left-hand grid to the right-hand grid. Edit the quantity in the " +
            "right-hand grid, or delete records. Verify that the example is protecting each " +
            "user's data from others by running the example in two different browsers (e.g. Firefox and IE) - " +
            "this creates distinct sessions with separate carts.</p>";
    ...
    public static class Factory extends AdvancedPanelFactory {
        ...
        public com.smartgwt.client.widgets.HTMLFlow getDisabledViewPanel() {
            final com.smartgwt.client.widgets.HTMLFlow htmlFlow = new com.smartgwt.client.widgets.HTMLFlow("<div class='explorerCheckErrorMessage'><p>This example is disabled because it requires the SQL Templating feature of " +
                    ...
                    "<p>Click <a href=\"http://www.smartclient.com/smartgwtee/showcase/#scripting_user_specific_data\" target=\"\">here</a> to see this example on SmartClient.com.</p></div>");
        ...
        public ShowcasePanel createShowcasePanel() {
            return new ScriptingUserSpecificDataSample();
        }
    }
    ...
    public Canvas getViewPanel() {
        ...
        DataSource carItemDS = DataSource.get("scripting_cartItem");
