...
public class JPASample extends ShowcasePanel {
    ...
    private static final String DESCRIPTION = "For pre-existing JPA entities, Smart GWT can automatically derive fully functional " +
            ...
            "configuration or Java code is required beyond the entity itself with the JPA mapping (which is " +
            "a set of samples intended to represent a pre-existing JPA entity).<p/>" +
            "To search, use the controls above the grid's header. Note that data paging is automatically " +
            "enabled. Simply scroll down to load data on demand. Click on the red icon next to each record to " +
